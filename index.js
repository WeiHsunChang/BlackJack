...
function fnAddcard() {
    ...
    dealer_card.innerHTML += '<img src="imgs/PokerCard/' + deck_obj[dealer_hand[dealer_hand.length - 1]] + '">'
    player_card.innerHTML += '<img src="imgs/PokerCard/' + deck_obj[player_hand[player_hand.length - 1]] + '">'
    //fnCountscore(d_f, p_f, dealer_hand.length - 1)
    fnCountDealer(d_f, dealer_hand.length - 1)
    fnCountPlayer(p_f, player_hand.length - 1)
    fnVersus(false)
}

async function fnFold() {
    while (Math.max(dealer_score, dealer_score2) < 17 || (Math.max(dealer_score, dealer_score2) > 21 && Math.min(dealer_score, dealer_score2) < 17)) {
        dealer_hand.push(deck.pop())
        dealer_card.innerHTML += '<img src="imgs/PokerCard/' + deck_obj[dealer_hand[dealer_hand.length - 1]] + '">'
        await fnCountDealer(d_f, dealer_hand.length - 1)
    }
    fnVersus(true)
}
    ...
    if (p == 21) {
        ...
        if (p > d) {
            ...
            for (let i = 0; i < dealer_hand.length; i++) {
                dealer_card.innerHTML += '<img src="imgs/PokerCard/' + deck_obj[dealer_hand[i]] + '">'
            }
            alert("和局")
        }
    }
